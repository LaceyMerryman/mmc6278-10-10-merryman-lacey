  ...
  describe('GET /pokemon - pokemon info page', () => {
    ...
    it('/pokemon?name=pokemonName should render pokemon image', async () => {
      const res = await request(app)
        ...
        .expect(200)
      const { window: { document } } = new JSDOM(res.text)
      const imgEl = document.querySelector('img[alt]')
    ...
    it('/pokemon?name=pokemonName should render pokemon types', async () => {
      ...
      expect(typesEl.textContent).to.include('ghost')
      expect(typesEl.textContent).to.include('poison')
    })
    ...
    it('should render "Pokemon not found" if given non-existent pokemon', async () => {
      pokeStub.restore()
      const errorStub = sinon.stub(pokeUtil, 'getPokemon').rejects(new Error('oh no'))
     
      await request(app)
        .get('/pokemon?name=banana')
        .expect(200)

      expect(errorStub.calledOnceWith('banana')).to.be.true
    })
  })
})
